...
public partial class EntityManager
{
    ...
    /// </summary>
    internal void AddComponentRange(EntityUid uid, List<ComponentType> compTypes)
    {
        DebugTools.Assert(compTypes.Count > 0);
        _world.AddRange(uid, compTypes);
    }

    internal void RemoveComponentRange(EntityUid uid, List<ComponentType> compTypes)
    {
        DebugTools.Assert(compTypes.Count > 0);
        _world.RemoveRange(uid, CollectionsMarshal.AsSpan(compTypes));
    }
}
