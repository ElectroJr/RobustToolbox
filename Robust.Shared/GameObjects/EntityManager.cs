...
namespace Robust.Shared.GameObjects
{
    ...
    public partial class EntityManager : IEntityManager
    {
        ...
        public virtual void FlushEntities()
        {
            ...
            var mapEnumerator = AllEntityQueryEnumerator<MapComponent>();
            while (mapEnumerator.MoveNext(out var map, out _))
            {
                try
                {
                    DeleteEntity(map);
            ...
            var enumerator = AllEntityQueryEnumerator<MetaDataComponent>();
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal void LoadEntity(EntityPrototype? prototype, EntityUid entity, IEntityLoadContext? context)
        {
            ...
            // Lort forgiv
            var types = new List<ComponentType>(count);
            var comps = new List<IComponent>(count);
            var compRegs = new List<ComponentRegistration>(count);
            Archetype arc;
            ...
            if (prototype != null)
            {
                foreach (var (name, entry) in prototype.Components)
                {
                    ...
                    var compType = reg.Idx.Type;
                    var comp = EntityPrototype.EnsureCompExistsAndDeserialize(entity, reg, _componentFactory, this, _serManager, name, fullData, context as ISerializationContext, metadata);
            ...
            if (context != null)
            {
                foreach (var name in context.GetExtraComponentTypes())
                {
                    ...
                    var reg = _componentFactory.GetRegistration(name);
                    var compType = reg.Idx.Type;
                    var comp = EntityPrototype.EnsureCompExistsAndDeserialize(entity, reg, _componentFactory, this, _serManager, name, data, context as ISerializationContext, metadata);
