...
void main()
{
    ...
    // how close does the line defined by the two points come to the origin/light source?
    // If it is to close, we effectively limit the size of the light source to avoid it intersecting the occluder.
    highp vec3 lineee = vec3(
    pointB.y - pointA.y, // a
    pointA.x - pointB.x, // b
    pointA.x * pointB.y - pointA.y * pointB.x // c
    );
    float closest = abs(lineee.z)/sqrt(lineee.x*lineee.x + lineee.y*lineee.y);
    ...
    // i.e., shrink pixel borders slightly.
    ...
    switch (pointId)
    {
        ...
        // ANTUMBRA PREVENTION:
        // We simply make the penumbra offset one directional.
        // I.e., if you picture the penumbra as an isocoles triangle, full in shadow on one side and in light on the other
        // We cut that triangle in half and assume that the center line is full in shadow
        // This prevents any possible antumbra from forming.
        /*case 2:
        offset = pointA/length(pointA) * 0.5 * lightLength;
        offset = vec2(offset.y, -offset.x);
        break;
    ...
    occlusion *= length(point-shadowOrigin);
