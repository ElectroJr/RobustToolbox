    ...
    internal sealed class GameStateProcessor : IGameStateProcessor
    {
        ...
        public void AddNewState(GameState state)
        {
            ...
            if (state.FromSequence == GameTick.Zero)
            {
                ...
                if (LastFullState == null || (LastFullState != null && LastFullState.ToSequence < state.ToSequence))
                {
                    LastFullState = state;
        ...
        private bool TryGetFullState([NotNullWhen(true)] out GameState? curState, out GameState? nextState)
        {
            ...
            var lastTick = LastFullState.ToSequence - 1;
            var nextTick = LastFullState.ToSequence + 1;
            for (var i = 0; i < _stateBuffer.Count; i++)
            {
                var state = _stateBuffer[i];

                if (state.ToSequence < lastTick) 
                {
        ...
        public List<(GameTick Tick, List<EntityUid> Entities)> GetEntitiesToDetatch(GameTick toTick)
        {
        ...
        private bool TryGetDeltaState([NotNullWhen(true)] out GameState? curState, out GameState? nextState)
        {
            ...
            var targetTick = _timing.LastProcessedTick + 1;
            var nextTick = targetTick + 1;
            ...
            for (var i = 0; i < _stateBuffer.Count; i++)
            {
                ...
                if (state.ToSequence == targetTick)
                {
                    curState = state;
                ...
                else if (state.ToSequence > targetTick)
                {
            ...
            if (curState == null && futureStateLowestFromSeq != null && futureStateLowestFromSeq <= _timing.LastRealTick)
            {
                ...
                curState = ExtrapolateState(_highestFromSequence, targetTick, lastStateInput);
