...
public sealed class EntitySerializer : ISerializationContext, ITypeSerializer<EntityUid, ValueDataNode>
{
    ...
    /// </summary>
    public EntityUid Truncate;

    public SerializationOptions Options;

    /// <summary>
    /// True if any errors were logged during serialization.
    /// </summary>
    public bool Error;
    ...
    /// <summary>
    /// Set of components that shouldn't get serialized.
    /// </summary>
    private readonly FrozenSet<Type> _ignoredComponents;
        ...
        IConfigurationManager conf)
    {
        ...
        _metaName = factory.GetComponentName(typeof(MetaDataComponent));
        _xformName = factory.GetComponentName(typeof(TransformComponent));
        ...
        _ignoredComponents = refMan.FindTypesWithAttribute<NonSerializedComponentAttribute>().ToFrozenSet();
    ...
    /// entity's parents, but depending on the setting of <see cref="SerializationOptions.MissingEntityBehaviour"/> this may
    /// auto-include additional entities.
    /// </summary>
    ...
    private void InitializeTileMap(EntityUid root)
    {
        ...
        foreach (var (origId, prototypeId) in savedMap)
        {
            if (_tileDef.TryGetDefinition(prototypeId, out var definition))
                _tileMap.Add(definition.TileId, origId);
        }
    }
    ...
    private void ProcessAutoInclude()
    {
        ...
        if (Options.MissingEntityBehaviour == MissingEntityBehaviour.AutoInclude)
        {
    ...
    private void SerializeEntityInternal(EntityUid uid)
    {
        ...
        switch (meta.EntityLifeStage)
        {
            case <= EntityLifeStage.Initializing:
                LogError($"Encountered an uninitialized entity: {EntMan.ToPrettyString(uid)}");
                break;
            case >= EntityLifeStage.Terminating:
                LogError($"Encountered terminating or deleted entity: {EntMan.ToPrettyString(uid)}");
                break;
        }
        ...
        if (meta.EntityLifeStage == EntityLifeStage.MapInitialized)
        {
            if (Options.ExpectPreInit)
                LogError($"Expected all entities to be pre-mapinit, but encountered post-init entity: {EntMan.ToPrettyString(uid)}");
        ...
        {
            LogError($"Encountered a no-rotation entity with non-zero local rotation: {EntMan.ToPrettyString(uid)}");
        ...
        foreach (var component in EntMan.GetComponentsInternal(uid))
        {
            ...
            if (_ignoredComponents.Contains(compType))
                continue;
            ...
            if (cache != null && cache.TryGetValue(compName, out protoMapping))
            {
    ...
    public Category GetCategory()
    {
        switch (Options.Category)
        {
            ...
            case Category.Map:
                return Maps.Count == 1 ? Category.Map : Category.Unknown;
        ...
        ISerializationContext? context = null)
    {
        if (Truncate != EntityUid.Invalid && value == Truncate)
            LogError($"{EntMan.ToPrettyString(CurrentEntity)} is attempting to serialize references to a truncated parent entity {EntMan.ToPrettyString(Truncate)}.");
        ...
        if (CurrentComponent == _xformName)
        {
            ...
            if (Options.ErrorOnOrphan && CurrentEntity != null && value != Truncate)
                LogError($"Serializing entity {EntMan.ToPrettyString(CurrentEntity)} without including its parent {EntMan.ToPrettyString(value)}");
        ...
        if (value == EntityUid.Invalid)
        {
            if (Options.MissingEntityBehaviour != MissingEntityBehaviour.Ignore)
                LogError($"Encountered an invalid entityUid reference.");
